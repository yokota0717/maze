...
bool Map::LoadMap(string & path, short id)
{
	path = "./data/map/" + path;
	//バイナリファイルを読み取り専用で開く
	ifstream fin(path, ios::in | ios::binary);
...
{
	if (map[y][x] == State::Non) { return; }
	//ウィンドウ座標に変換
	Math::Vec pos(200 + (x / 2) * 100, 100 + (y / 2) * 100);
	if (x % 2 == 0 && y % 2 == 0) {
		//柱
		DrawCircle(pos.x, pos.y, 5, lineColor);
	}
	if (x % 2 != 0 && y % 2 == 0) {
		//横
		DrawLine(pos.x, pos.y, pos.x + 100, pos.y, lineColor, 5);
	}
	if (x % 2 == 0 && y % 2 != 0) {
		//縦
		DrawLine(pos.x, pos.y, pos.x, pos.y + 100, lineColor, 5);
	}
}
...
void Map::renderBox(int x, int y)
{
	...
	//ウィンドウ座標に変換
	Math::Vec pos(200 + (x / 2) * 100, 100 + (y / 2) * 100);
	DrawBox(pos.x + 5, pos.y + 5, pos.x + 95, pos.y + 95, boxColor, false);
}
...
void Field::init()
{
	map.clear();
	map.resize(4);
	LoadMap(vector<string>{(string)"map1.csv", (string)"map2.csv", (string)"map3.csv", (string)"map4.csv"});
	activeMapID_ = 0;
}
...
bool Field::LoadMap(vector<string> path)
{
	for (int i = 0; i < 4; ++i) {
